...
def predictSingle():
    ...
    prediction = 0
    TTA = 1
    predict_df = pd.read_csv(os.path.join(DATA_FOLDER, "input.csv"))
    print(predict_df)
    for i in range(5):
        print(predict_df.iloc[0][i])
    ...
    predict_loader = DataLoader(dataset=predict_data, batch_size=1, shuffle=False, num_workers=2)
